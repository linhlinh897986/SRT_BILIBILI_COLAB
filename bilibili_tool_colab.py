...
def merge_sentence_logic(content: str, gap_threshold_ms: int):
    ...
    def finalize_block(fragments):
        ...
        # 1. Join all text fragments
        full_text = "".join(f['text'] for f in fragments)

        # 2. Find the first speaker tag
        first_tag_match = re.search(r'(\[SPEAKER_\d+\]:)', full_text)
        first_tag = first_tag_match.group(1) if first_tag_match else ""

        # 3. Remove all speaker tags from the text, replacing with a space
        text_without_tags = re.sub(r'\[SPEAKER_\d+\]:\s*', ' ', full_text).strip()
        
        # 4. Fix long punctuation chains
        text_without_tags = re.sub(r'[\.,。，?!…]{2,}', '.', text_without_tags)
        
        # 5. Re-assemble the final text
        if first_tag:
            # Ensure the first tag is not duplicated if it was not removed
            text_without_tags = text_without_tags.replace(first_tag, "").strip()
            final_text = f"{first_tag} {text_without_tags}"
        else:
            final_text = text_without_tags
